...
const handleClickCamera = async () => {
  ...
  const canvas = await html2canvas(appRoot, {
    ...
    onclone: (clonedDoc) => {
      ...
      const details = clonedDoc.querySelectorAll<HTMLElement>(
        '[class*="variationContainer"], [class*="divDown"]',
      );
...
export const Header = memo(
  ({ onMapClick, toggleSearch, handleCloseSearch }: HeaderProps) => {
    ...
    return (
      <header className={styles['header']}>
        <div className={styles['header-actions']}>
          <div className={styles['header-actions-primary']}>
            <Tooltip title="לוח בקרה" arrow>
              <i
                className={`bi bi-speedometer2 ${styles['header-icon']}`}
            ...
            <Tooltip title="מקרא" arrow>
              <i
                className={`bi bi-map ${styles['little']} ${styles['header-icon']}`}
                onClick={onMapClick}
              />
            </Tooltip>
            <Tooltip title="התראות" arrow>
              <i
                className={`bi bi-bell ${styles['little']} ${styles['header-icon']}`}
              />
            </Tooltip>
            <Tooltip title="חיפוש" arrow>
              <i
                className={`bi bi-search ${styles['little']} ${styles['header-icon']}`}
          ...
          <div className={styles['header-actions-secondary']}>
            <Tooltip title="עריכה" arrow>
              <i
                className={`bi bi-pencil ${styles['little']} ${styles['header-icon']}`}
              />
            </Tooltip>
            <Tooltip title="סימניה" arrow>
              <i
                className={`bi bi-bookmark ${styles['little']} ${styles['header-icon']}`}
              />
            </Tooltip>
            <Tooltip title="רענון" arrow>
              <i
                className={`bi bi-arrow-clockwise ${styles['header-icon']}`}
            ...
            <Tooltip title="מצלמה" arrow>
              <i
                className={`bi bi-camera ${styles['header-icon']}`}
            ...
            <Tooltip title="הגדרות צפייה" arrow>
              <i
                className={`bi bi-toggles2 ${styles['header-icon']}`}
        ...
        <i
          className={`${styles['control-icon']} bi bi-grid ${styles['header-icon']}`}
        />
