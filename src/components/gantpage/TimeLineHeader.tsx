  ...
  const generateTicks = (start: Dayjs | null, end: Dayjs | null) => {
    ...
    if (diffInDays <= 15) {
      ...
      while (!current.isAfter(end.add(1, 'day'))) {
        ...
        const yearHasChanged = currentYear !== previousYear;

        ticks.push({
          label: current.format('D MMM'),
          yearBadge:
            isFirstTick || yearHasChanged ? currentYear.toString() : undefined,
    ...
    <div className={styles['timeline-header']}>
      ...
      <div className={styles['ticks-container']}>
        {dates.map((item, i) => (
          ...
          >
            <span className={styles['tick-label']}>
              {item.label + ' '}
              {item.yearBadge && (
                <span className={styles['year-badge']}>{item.yearBadge}</span>
              )}
            </span>
